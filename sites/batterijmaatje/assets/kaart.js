...
(function (root, factory) {
  ...
  /* ---- De dagmaat -------------------------------------------------------
     ...
     De rekensom en de teksten staan in assets/dagmaat.js, want een
     productpagina laadt dit bestand niet en heeft die uitleg wel nodig. Hier
     alleen de vertaling van een batterij naar de twee getallen die de dagmaat
     nodig heeft: de opgegeven capaciteit, en of vaststaat dat dat de bruikbare
     maat is. */
  const dagmaatMaat = (b) => ({
    capaciteit: b.capaciteit_kwh,
    bevestigd: Prijs.capaciteitBevestigd(b),
    id: b.id,
  });
  ...
  function dagmaatUitlegHtml(b, verbruik) {
    ...
    const d = Dagmaat.bereken(maat, verbruik);
    if (!d) return "";
    return `<div class="dagmaat-uitleg" data-capaciteit="${b.capaciteit_kwh}" data-bevestigd="${d.bevestigd ? "1" : "0"}" data-batterij="${escapeHtml(b.id)}">
      ${Dagmaat.uitlegHtml(maat, verbruik)}
    </div>`;
  }
  ...
  function kaartHtml(b, opties) {
    ...
    <article class="batterij-kaart" data-id="${escapeHtml(b.id)}">
      ...
      <div class="kaart-specs">
        <div class="spec"><span class="spec-label"><a class="term-link" href="uitleg.html#capaciteit" title="Wat is capaciteit (kWh)? Lees de uitleg">Capaciteit</a></span><span class="spec-waarde">${capaciteit}${dagmaatHtml(b, o.verbruik)}</span></div>
